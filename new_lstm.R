...
crudes <- read.csv(file="C:/Users/avocado/Desktop/crude_oil/lstm/crude_oil_set.csv",header=TRUE,sep=",")

head(crudes)

crudes$index = as.Date(crudes$index)
...
crude <- crude[complete.cases(crude),]
...
p2 <- crude %>%
  ...
  labs(
    ...
    caption = "Crude Oil Prices over the last year and change"
  )

p_title <- ggdraw() + 
  draw_label("Crude Oil Prices", size = 18, fontface = "bold", 
             colour = palette_light()[[1]])

plot_grid(p_title, p1, ncol = 1, rel_heights = c(0.1, 1, 1))

plot_grid(p_title, p2, ncol = 1, rel_heights = c(0.1, 1, 1))
...
# Prediction shit
# how well was the model able to absorb the characteristics of the
# data set
...
# compute RMSE over all sequences of predictions
